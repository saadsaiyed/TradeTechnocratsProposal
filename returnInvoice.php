<?php
    ...
    if (!empty($_POST)){    
        ...
        $Company_Name = str_replace("^", "'", $temp[1]);
        ...
        if($result->num_rows > 0){
            ...
            $query = "INSERT INTO Return_Invoice (Customer_ID, Total_Count, Note) VALUES ('$Customer_ID', '$Total_Count', '$Note')";
            $Invoice_ID  = runQueryGiveId($query);
            ...
            for ($i=1; $i <= ((count($_POST) - 5) / 4); $i++) { 
                ...
                if ($Barcode_ID != "" && $Barcode_ID != " ") {
                    runQuery("INSERT INTO `B_Return_Barcode` (`Invoice_ID`, `Barcode_ID`, `Count`) VALUES ('$Invoice_ID', '$Barcode_ID', '$Count')");
                    
                    //New adjustment is orginial adjustment of this Barcode + thisReturnItem's count
                    $New_Adjustment = ((int)mysqli_fetch_array(runQuery("SELECT Adjustment FROM Barcode WHERE `Barcode_ID`='$Barcode_ID'"))["Adjustment"] ) + $Count;
                    runQuery("UPDATE Barcode SET Adjustment = '$New_Adjustment' WHERE `Barcode_ID`='$Barcode_ID'");
                }
            }
            $info = "Invoice Saved Succesfully with Invoice_Num = " . $Invoice_Num;
        }
        else{
            $info = "Cannot find any Customer with value '$Customer_Name'";
        }
    }
...
<html lang="en">
    <head>
        ...
        <title>Return Invoice | TTParikh</title>
                ...
                <div class="grid item2">
                    <div class="inside" id="topTableID">
                        ...
                        <table>
                            ...
                            <tr>
                                <td><label for="invoice_name">Customer Name - Company :</label></td>
                                ...
                                <td><input type="text-area" value="<?=$Note?>" name="note" id="note"></td>
                            </tr>
                            ...
                            <tbody id="addInside_tr_20">
                                <?php
                                    $stringToDisplay = '';
                                        for ($i=1; $i <= 200; $i++) { 
                                            $stringToDisplay .= '<tr>';
                                            $stringToDisplay .= '<td><input type="text" autocomplete="off" id="barcode_'.$i.'" name="barcode_'.$i.'" onchange="barcodeOnChange('.$i.')"></td>';
                                            ...
                                            $stringToDisplay .= '<td><input type="text" autocomplete="off" id="count_'.$i.'" name="count_'.$i.'" onchange="quantityDisplay()" tabindex="-1"></td>';
